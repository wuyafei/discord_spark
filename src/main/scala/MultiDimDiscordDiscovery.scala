/**
 * Created by wuyafei on 15/8/9.
 */
import org.apache.spark.{SparkContext, SparkConf}
import util.control.Breaks._

object MultiDimDiscordDiscovery {
  def main (args: Array[String]): Unit = {
    val data_file = "data/xmitdb_orig.txt"
    val conf = new SparkConf().setAppName("MultiDim Discord Discovery").setMaster("local[2]")
    val sc = new SparkContext(conf)
    val window = 360
    val range = 1.55
    val ts = sc.textFile(data_file, 4).map(_.split(" ")).map(x => x.map(_.toDouble)).zipWithIndex().map(x => (x._2, x._1))
    ...
    def distance(d1:Array[Double],d2:Array[Double]):Array[Double] = {
      val pair = d1.zip(d2)
      val sum = pair.map(x => math.pow(x._1 - x._2, 2))
      sum
    }

    def multidim_distance(d1:Array[Array[Double]], d2:Array[Array[Double]]):Double = {
      val pair = d1.zip(d2)
      val sum = pair.map(x => distance(x._1, x._2)).sum.map(math.sqrt(_)).sum
      math.sqrt(sum)
    }
    ...
    val candidates = partitioned.mapPartitionsWithIndex((idx,par) => {
      ...
      val len = data.length
      var cndd = Array((-1,-1L,0.0,Array(Array(0.0))))     //(partition_idx, record_idx, nn_dist, time_series)
      for(i <- 0 to (len - window)){
        breakable {
          var nn_dist = 9999999999.9
          for (j <- 0 to (len - window)) {
            if (math.abs(i - j) >= window) {
              val dist = multidim_distance(data.slice(i, i + window), data.slice(j, j + window))
              if (dist < range)
                break()
              if(dist < nn_dist)
                nn_dist = dist
        ...
        breakable{
          ...
          for(j <- 0 to (len-window)){
            if(math.abs(index(j) - seq_candidate._2) >= window){
              val dist = multidim_distance(seq_candidate._4, data.slice(j, j+window))
              if(dist < range){
                break()
              }
              if(dist < nn_dist)
